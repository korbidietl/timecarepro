...
def capture_signature():
    if request.method == 'POST':
        ...
        if target_function == 'create_time_entry':
            return render_template('templates/FMOF030_create_time_entry.html', signature_data=signature_data)
        elif target_function == 'edit_time_entry':
            return render_template('templates/FMOF050_edit_time_entry.html', signature_data=signature_data)
        elif target_function == 'login':  # FV100:
            return render_template('login', signature_data=signature_data)  # FV100


    else:
        return render_template('templates/FS020_sign_capture.html')
